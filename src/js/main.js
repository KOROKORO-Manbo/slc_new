  ...
  (function() {
    ...
    scrolltotop.init();         // 「ページトップへ戻る」初期化
      ...
      if (!window.localStorage) {                              // ルーレットメニュー説明モーダル表示
        ...
        var t = window.localStorage.getItem("todo-first20200412");
    ...
    $('.neverShow').click(function(e){                      // ルーレットメニュー説明モーダル画面
      if ($(this).prop('checked')) {                        // 非表示＝false
        window.localStorage.setItem("todo-first20200412", false);
      } else {
        window.localStorage.setItem("todo-first20200412", true);
      }
    });
